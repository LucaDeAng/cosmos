/**
 * Open Beauty Facts Source
 *
 * Integrates with Open Beauty Facts API for cosmetic/beauty product enrichment.
 * Contains cosmetic, hygiene, and personal care products with:
 * - Barcode (EAN/GTIN)
 ...
 * - Product images
 *
 * API is free and unlimited: https://world.openbeautyfacts.org/api/v2
 */

import type { SectorCode, EnrichmentResult, EnrichmentContext, OpenBeautyFactsProduct } from '../types';
...
interface OpenBeautyFactsApiResponse {
  status: number;
...
export class OpenBeautyFactsSource implements EnrichmentSource {
  name = 'open_beauty_facts' as const;
  supportedSectors: SectorCode[] = ['consumer_goods'];
  priority = 2;
  ...
  private config = SOURCE_CONFIGS.open_beauty_facts;
  ...
  async initialize(): Promise<void> {
    console.log('✅ Open Beauty Facts source initialized');
  }

  /**
   * Enrich an item with Open Beauty Facts data
   */
  async enrich(item: ExtractedItem, context: EnrichmentContext): Promise<EnrichmentResult> {
    ...
    return this.emptyResult('No matching product found in Open Beauty Facts');
  }

  /**
   * Get product by barcode
   */
  async getByBarcode(barcode: string, skipCache = false): Promise<OpenBeautyFactsProduct | null> {
    const cacheKey = `barcode:${barcode}`;

    // Check cache first
    if (!skipCache) {
      const cached = await this.cache.get<OpenBeautyFactsProduct>(this.name, cacheKey);
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`Open Beauty Facts API error: ${response.status}`);
        return null;
      }

      const data = await response.json() as OpenBeautyFactsApiResponse;
      ...
      console.error('Open Beauty Facts barcode lookup failed:', error);
      return null;
    }
  }
  ...
  async searchByName(
    ...
    limit = 5,
    skipCache = false
  ): Promise<OpenBeautyFactsProduct[]> {
    ...
    try {
      ...
      url.searchParams.set('fields', 'code,product_name,brands,categories_tags,ingredients_text,certifications_tags,image_front_url');
      ...
      if (!response.ok) {
        console.warn(`Open Beauty Facts search error: ${response.status}`);
        return [];
      }

      const data = await response.json() as OpenBeautyFactsSearchResponse;
  ...
  private findBestMatch(
    ...
    products: OpenBeautyFactsProduct[]
  ): { product: OpenBeautyFactsProduct; confidence: number } | null {
    if (products.length === 0) return null;

    const nameLower = name.toLowerCase();
    const brandLower = brand?.toLowerCase();

    let bestMatch: OpenBeautyFactsProduct | null = null;
    let bestScore = 0;
    ...
    for (const product of products) {
      ...
      if (product.ingredients_text) score += 0.1; // INCI ingredients are valuable
      if (product.certifications_tags?.length) score += 0.05;
  ...
  /**
   * Map Open Beauty Facts product to EnrichmentResult
   */
  private mapToEnrichmentResult(
    ...
    // Ingredients (INCI format for cosmetics)
    if (product.ingredients_text) {
      enrichedFields.ingredients_inci = product.ingredients_text;
      fieldsEnriched.push('ingredients_inci');
    }

    // Certifications (cruelty-free, vegan, organic, etc.)
    if (product.certifications_tags?.length) {
      enrichedFields.certifications = product.certifications_tags.map(this.formatTag);
    ...
    return {
      ...
      reasoning: [
        ...
        product.certifications_tags?.length
          ? `Certifications: ${product.certifications_tags.map(this.formatTag).join(', ')}`
          : '',
      ].filter(Boolean),
    };
  }
  ...
  /**
   * Format Open Beauty Facts tag (remove language prefix)
   */
...
// Singleton instance
let sourceInstance: OpenBeautyFactsSource | null = null;

export function getOpenBeautyFactsSource(): OpenBeautyFactsSource {
  if (!sourceInstance) {
    sourceInstance = new OpenBeautyFactsSource();
  }
  return sourceInstance;
}

export default OpenBeautyFactsSource;
